Sometimes, the default settings in a framework and associated tools can
lead to trouble. The boolean type in Rails is one such.
...
that works in all cases, but a common practice is to set both a default
value *and* disallow nulls in the field, like so:

{% highlight ruby %} add\_column :widgets, :frangible, :boolean,
default: false, null: false {% endhighlight %}
